...
const App = //*********************************************************
  ...
  (function() {
    ...
    var INSTALLATION_ID = 0,
      ...
      VSO_URL_FORMAT = "https://%@.visualstudio.com",
      ...
      VSO_ZENDESK_LINK_TO_TICKET_ATTACHMENT_PREFIX =
        "ZendeskLinkTo_Attachment_Ticket_",
      VSO_WI_TYPES_WHITE_LISTS = [
        "Bug",
    ...
    return {
      ...
      events: {
        ...
        "click .linkModal button.reloadQueriesBtn":
          "onLinkReloadQueriesButtonClick",
      ...
      requests: {
        ...
        getVsoProjectWorkItemTypes: function(projectId) {
          return this.vsoRequest(
            helpers.fmt("/%@/_apis/wit/workitemtypes", projectId)
          );
        },
        getVsoProjectAreas: function(projectId) {
          return this.vsoRequest(
            helpers.fmt("/%@/_apis/wit/classificationnodes/areas", projectId),
            {
              $depth: 9999
            }
          );
        ...
        getVsoProjectWorkItemQueries: function(projectName) {
          return this.vsoRequest(
            helpers.fmt("/%@/_apis/wit/queries", projectName),
            {
              $depth: 2
            }
          );
        ...
        getVsoWorkItem: function(workItemId) {
          return this.vsoRequest(
            helpers.fmt("/_apis/wit/workItems/%@", workItemId),
            {
              $expand: "relations"
            }
          );
        ...
        getVsoWorkItemQueryResult: function(projectName, queryId) {
          return this.vsoRequest(
            helpers.fmt("/%@/_apis/wit/wiql/%@", projectName, queryId)
          );
        },
        createVsoWorkItem: function(projectId, witName, data) {
          return this.vsoRequest(
            helpers.fmt("/%@/_apis/wit/workitems/$%@", projectId, witName),
            undefined,
            {
              type: "PUT",
      ...
      onGetVsoProjectsDone: function(projects) {
        this.vm.projects = _.sortBy(
          this.vm.projects.concat(
      ...
      onGetLinkedVsoWorkItemsDone: function(data) {
        ...
        _.each(
          ...
          function(workItem) {
            workItem.title = helpers.fmt(
              "%@: %@",
              workItem.id,
              this.getWorkItemFieldValue(workItem, "System.Title")
            );
          }.bind(this)
        );
      ...
      onAppActivated: function(data) {
        if (data.firstLoad) {
          ...
          if (!this.vm.isAppLoadedOk) {
            ...
            try {
              this.vm.fieldSettings = JSON.parse(
                this.setting("vso_field_settings") || DEFAULT_FIELD_SETTINGS
              );
            } catch (ex) {
              services.notify(
                this.I18n.t("errorReadingFieldSettings"),
                "alert"
              );
      ...
      onNewVsoWorkItemTypeChange: function() {
        ...
        var workItemType = this.getWorkItemTypeByName(
          project,
          $modal.find(".type").val()
        ); //Check if we have severity

        if (
          this.hasFieldDefined(workItemType, "Microsoft.VSTS.Common.Severity")
        ) {
      ...
      onNewCopyDescriptionClick: function(event) {
        ...
        this.$(".newWorkItemModal .description").val(
      ...
      onNewCopyTemplateClick: function(event) {
        event.preventDefault();
        this.$(".newWorkItemModal .description").val(
          this.setting("vso_wi_description_template")
        );
      },
      onNewWorkItemAcceptClick: function() {
        var $modal = this.$(".newWorkItemModal").modal(); //check project

        var proj = this.getProjectById($modal.find(".project").val());

        if (!proj) {
          return this.showErrorInModal(
            $modal,
            this.I18n.t("modals.new.errProjRequired")
          );
        } // read area id

        var areaId = $modal.find(".area").val(); //check work item type
        ...
        if (!workItemType) {
          return this.showErrorInModal(
            $modal,
            this.I18n.t("modals.new.errWorkItemTypeRequired")
          );
        } //check summary
        ...
        if (!summary) {
          return this.showErrorInModal(
            $modal,
            this.I18n.t("modals.new.errSummaryRequired")
          );
        }
        ...
        if (areaId) {
          operations.push(
            this.buildPatchToAddWorkItemField("System.AreaId", areaId)
          );
        }

        if (
          this.hasFieldDefined(
            workItemType,
            "Microsoft.VSTS.Common.Severity"
          ) &&
          $modal.find(".severity").val()
        ) {
          operations.push(
            this.buildPatchToAddWorkItemField(
              "Microsoft.VSTS.Common.Severity",
              $modal.find(".severity").val()
            )
          );
        ...
        if (
          this.hasFieldDefined(workItemType, "Microsoft.VSTS.TCM.ReproSteps")
        ) {
          operations.push(
            this.buildPatchToAddWorkItemField(
        ...
        operations = operations.concat(
          this.buildPatchToAddWorkItemAttachments(attachments)
        );
        this.showSpinnerInModal($modal);
        this.ajax("createVsoWorkItem", proj.id, workItemType.name, operations)
          .done(
            function(data) {
              var newWorkItemId = data.id; //sanity check due tfs returning 200 ok  but with exception

              if (newWorkItemId > 0) {
                this.linkTicket(newWorkItemId);
              }

              services.notify(
                this.I18n.t("notify.workItemCreated").fmt(newWorkItemId)
              );
              this.getLinkedVsoWorkItems(
                function() {
      ...
      onShowDetailsClick: function(event) {
        ...
        $modal
          ...
          .html(this.I18n.t("modals.details.loading"));
        $modal.find(".modal-body").html(this.renderTemplate("loading"));
        ...
        $modal
          .find(".modal-body")
          .html(this.renderTemplate("details", workItem));
      },
      ...
      onLinkQueryButtonClick: function() {
        ...
        var drawQueryResults = function(results, countQueryItemsResult) {
          var workItems = _.map(
            ...
            function(workItem) {
              return {
                id: workItem.id,
                type: this.getWorkItemFieldValue(
                  workItem,
                  "System.WorkItemType"
                ),
        ...
        this.showSpinnerInModal($modal);
        this.ajax(
          "getVsoWorkItemQueryResult",
          this.getProjectById(projId).name,
          queryId
        )
          .done(
            function(data) {
              var getWorkItemsIdsFromQueryResult = function(result) {
                if (
                  result.queryType === "oneHop" ||
                  result.queryType === "tree"
                ) {
          ...
          .fail(
            function(jqXHR, textStatus, errorThrown) {
              this.showErrorInModal(
                $modal,
                this.getAjaxErrorMessage(
                  jqXHR,
                  this.I18n.t("modals.link.errCannotGetWorkItem")
                )
              );
            }.bind(this)
          );
      ...
      onLinkAcceptClick: function(event) {
        ...
        if (!/^([0-9]+)$/.test(workItemId)) {
          return this.showErrorInModal(
            $modal,
            this.I18n.t("modals.link.errWorkItemIdNaN")
          );
        }
        ...
        if (this.isAlreadyLinkedToWorkItem(workItemId)) {
          return this.showErrorInModal(
            $modal,
            this.I18n.t("modals.link.errAlreadyLinked")
          );
        }
        ...
        var updateWorkItem = function(workItem) {
          ...
          var finish = function() {
            ...
            services.notify(
              this.I18n.t("notify.workItemLinked").fmt(workItemId)
            );
            this.getLinkedVsoWorkItems(
              function() {
                this.closeModal($modal);
              ...
              .fail(
                function(jqXHR) {
                  this.showErrorInModal(
                    ...
                    this.getAjaxErrorMessage(
                      jqXHR,
                      this.I18n.t("modals.link.errCannotUpdateWorkItem")
                    )
                  );
                }.bind(this)
              );
      ...
      onUnlinkAcceptClick: function(event) {
        ...
        var updateWorkItem = function(workItem) {
          ...
          var finish = function() {
            ...
            services.notify(
              this.I18n.t("notify.workItemUnlinked").fmt(workItem.id)
            );
            this.getLinkedVsoWorkItems(
              function() {
                this.closeModal($modal);
              ...
              .fail(
                function(jqXHR) {
                  this.showErrorInModal(
                    ...
                    this.getAjaxErrorMessage(
                      jqXHR,
                      this.I18n.t("modals.unlink.errUnlink")
                    )
                  );
                }.bind(this)
              );
      ...
      onNotifyAcceptClick: function() {
        ...
        if (!text) {
          return this.showErrorInModal(
            $modal,
            this.I18n.t("modals.notify.errCommentRequired")
          );
        }
        ...
        this.getLinkedVsoWorkItems(
          function(workItems) {
            ...
            this.when
              .apply(this, requests) //this.ajax('updateMultipleVsoWorkItem', updatePayload)
              .done(
                function() {
                  var ticketMsg = [
                    this.I18n.t("notify.message", {
              ...
              .fail(
                function(jqXHR) {
                  this.showErrorInModal(
                    $modal,
                    this.getAjaxErrorMessage(jqXHR)
                  );
                }.bind(this)
              );
          }.bind(this)
      ...
      drawWorkItems: function(data) {
        var workItems = _.map(
          ...
          function(workItem) {
            var tmp = this.attachRestrictedFieldsToWorkItem(
              workItem,
              "summary"
            );
            return tmp;
      ...
      drawQueriesList: function(select, projectId) {
        ...
        var drawNode = function(node, prefix) {
            ...
            return "<optgroup label='%@ %@'>%@</optgroup>".fmt(
              ...
              _.reduce(
                ...
                function(options, childNode, ix) {
                  return "%@%@".fmt(
                    options,
                    drawNode(childNode, prefix + (ix + 1) + ".")
                  );
                },
                ""
              )
          ...
          return "<option value='%@'>%@ %@</option>".fmt(
            node.id,
            prefix,
            node.name
          );
      ...
      authString: function(vso_username, vso_password) {
        ...
        return helpers.fmt(
          "Basic %@",
          this.store("auth_token_for_" + this.setting("vso_account"))
        );
      },
      vsoRequest: function(url, parameters, options) {
        ...
        var fixedHeaders = {
          ...
          Accept: helpers.fmt(
            ...
            this.getVsoResourceVersion(url)
          )
        };
      ...
      isAlreadyLinkedToWorkItem: function(id) {
        return _.contains(this.getLinkedWorkItemIds(), id);
      },
      loadProjectMetadata: function(projectId) {
        ...
        var loadWorkItemTypes = this.ajax(
          "getVsoProjectWorkItemTypes",
          project.id
        ).done(
          function(data) {
        ...
        var loadAreas = this.ajax("getVsoProjectAreas", project.id).done(
          function(rootArea) {
      ...
      loadProjectWorkItemQueries: function(projectId, reload) {
        ...
        return this.ajax("getVsoProjectWorkItemQueries", project.name).done(
          function(data) {
      ...
      getAjaxErrorMessage: function(jqXHR, errMsg) {
        ...
        if (jqXHR.responseJSON) {
          serverErrMsg =
            jqXHR.responseJSON.message || jqXHR.responseJSON.value.Message;
        } else {
          serverErrMsg = jqXHR.responseText.substring(0, 50) + "...";
        }

        var detail = this.I18n
          .t("errorServer")
          .fmt(jqXHR.status, jqXHR.statusText, serverErrMsg);
      ...
      buildAccountUrl: function() {
        ...
        if (
          loweredSetting.indexOf("http://") === 0 ||
          loweredSetting.indexOf("https://") === 0
        ) {
          baseUrl = setting;
        } else {
